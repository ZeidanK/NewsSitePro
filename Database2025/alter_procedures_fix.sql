...
-- 2. ALTER: Fix Search procedure to include repost count and proper view counts
...
-- 3. ALTER: Fix SavedArticles procedure to include proper interaction data
...
BEGIN
    ...
    SELECT 
        ...
        -- Aggregate likes count
        COALESCE(lc.LikesCount, 0) as LikesCount,
        -- Check if current user liked this article (will always be this user since it's their saved articles)
        CASE WHEN al.UserID IS NOT NULL THEN 1 ELSE 0 END as IsLiked,
...
-- 4. CREATE: New procedure for individual article with user context (if not exists)
...
-- 5. CREATE: Repost toggle procedure (if not exists)
...
BEGIN
    ...
    ELSE
    BEGIN
        INSERT INTO NewsSitePro2025_Reposts (OriginalArticleID, UserID, RepostDate)
