...
-- 5. CREATE: Following Feed procedure (if not exists)
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[NewsSitePro2025_sp_NewsArticles_GetFollowingFeed]') AND type in (N'P', N'PC'))
BEGIN
    EXEC('CREATE PROCEDURE NewsSitePro2025_sp_NewsArticles_GetFollowingFeed AS BEGIN SET NOCOUNT ON; END')
END
GO

ALTER PROCEDURE NewsSitePro2025_sp_NewsArticles_GetFollowingFeed
    @UserID INT,
    @PageNumber INT = 1,
    @PageSize INT = 10
AS
